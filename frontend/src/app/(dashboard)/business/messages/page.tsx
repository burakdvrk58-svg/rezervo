...
export default function BusinessMessagesPage() {
  ...
  const fetchContactsAndMessages = async (selectFirst = false) => {
    try {
      const res = await fetch(`/api/messages?userId=${academicianId}`)
      if (res.ok) {
        const data = await res.json()
  ...
  const fetchThread = async (contactId: string) => {
    try {
      const res = await fetch(`/api/messages?userId=${academicianId}&chatWith=${contactId}`)
      if (res.ok) {
        const data = await res.json()
        setMessages(data)
      }
  ...
  useEffect(() => {
    if (!academicianId || academicianId === 'u-academician') return
        ...
        {activeContact ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              <AnimatePresence initial={false}>
                {messages.map((msg) => {
                  const isMe = msg.senderId === academicianId
                  return (
                    <motion.div
                      key={msg.id}
          ...
          <div className="flex flex-1 flex-col items-center justify-center p-8 text-center text-slate-400">
            ...
            <p className="text-xs text-slate-400 mt-1 max-w-sm leading-relaxed">
              Mesajlaşmaya başlamak için sol taraftaki menüden öğrencinizi seçin.
            </p>
          </div>
        )}
      </div>
