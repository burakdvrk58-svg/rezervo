...
function Hero({ trustBadge, headline, subtitle, buttons, className = "" }: HeroProps) {
  ...
  const [bookingCount, setBookingCount] = useState(10240)

  useEffect(() => {
    const interval = setInterval(() => {
      setBookingCount((prev) => prev + Math.floor(Math.random() * 2) + 1)
    }, 5000)
  ...
  return (
    <section 
      ...
      className={`relative pt-36 pb-24 md:pt-48 md:pb-32 z-10 max-w-7xl mx-auto px-6 overflow-hidden ${className}`}
    >
      {/* Dynamic Cursor Glow (Hugo style micro-detail) */}
      <div 
        className="absolute inset-0 pointer-events-none z-0 transition-opacity duration-300 opacity-100 hidden lg:block"
        style={{
          background: `radial-gradient(circle 350px at ${heroMousePos.x}px ${heroMousePos.y}px, rgba(37,99,235,0.08), transparent)`
        }}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center relative z-10">
        ...
        <div className="lg:col-span-7 space-y-8">
          <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 backdrop-blur-md">
            ...
            <span className="text-xs font-semibold text-slate-200">{trustBadge.text}</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-2">
            <button 
              ...
              className="flex items-center justify-center gap-2 px-6 py-4 rounded-xl bg-blue-600 text-white font-semibold hover:bg-blue-500 transition-all duration-200 shadow-xl shadow-blue-600/20 group cursor-pointer"
            >
              {buttons.primary.text} <ArrowRight className="w-4.5 h-4.5 group-hover:translate-x-1 transition-transform" />
            </button>
            <button 
              onClick={buttons.secondary.onClick}
              className="flex items-center justify-center gap-2 px-6 py-4 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-white font-semibold transition-all duration-200 backdrop-blur-sm cursor-pointer"
            >
              {buttons.secondary.text}
            </button>
          </div>

          {/* Live Stats Ticker (Micro-Detail) */}
          <div className="flex items-center gap-8 pt-6 border-t border-white/5">
            <div>
              <span className="text-2xl font-bold text-white font-mono tracking-tight">
                {bookingCount.toLocaleString()}
              </span>
              <span className="text-xs text-slate-500 block mt-1">Yapılan Rezervasyon (Canlı)</span>
            </div>
            ...
            <div>
              ...
              <span className="text-xs text-slate-500 block mt-1">Memnuniyet Oranı</span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 relative flex items-center justify-center lg:justify-end">
          <div className="relative w-full max-w-[420px] aspect-[4/5] perspective-1000">
            ...
            <div className="absolute bottom-[15%] left-[-12%] glass rounded-2xl p-3 border border-white/15 shadow-2xl backdrop-blur-xl bg-black/60 flex items-center gap-2.5 transform translate-z-20">
              <div className="w-8.5 h-8.5 rounded-full bg-blue-500/10 border border-blue-500/25 flex items-center justify-center text-blue-400">
...
export default function LandingPage() {
  ...
  return (
    <div className="dark relative min-h-screen bg-black text-slate-300 font-sans selection:bg-white/20 selection:text-white overflow-x-hidden">
      ...
      {/* ── HERO SECTION WITH MOUSE FOLLOWER EFFECT ── */}
      <Hero
        trustBadge={{
          text: "Yeni Nesil Akademik Rezervasyon Sistemi",
          icons: ["🚀", "⭐", "✨"]
        }}
        headline={{
          line1: "Akademik Danışmanlığı",
          line2: "Yeniden Keşfedin."
        }}
        subtitle="Rezervo, üniversite öğrencilerinin danışman hocaları ile saniyeler içinde seans rezerve etmesini sağlayan, 3D vizyonlu ve canlı bildirim destekli yeni nesil akademik randevu portalıdır."
        buttons={{
          primary: {
            text: "Hemen Başlayın",
      ...
      <section 
        ...
        className="py-24 relative z-10 max-w-7xl mx-auto px-6 overflow-hidden"
      >
        {/* Cursor tracking ambient light inside Neden? */}
        <div 
          className="absolute inset-0 pointer-events-none z-0 transition-opacity duration-300 opacity-100 hidden lg:block"
          style={{
            background: `radial-gradient(circle 350px at ${whyMousePos.x}px ${whyMousePos.y}px, rgba(99,102,241,0.06), transparent)`
          }}
        />
      ...
      {/* ── CALENDAR INTEGRATIONS & MEETING ORBIT (Floating Ring Micro-Detail) ── */}
      <section id="integrations" className="py-24 border-t border-white/5 relative z-10 overflow-hidden bg-slate-950/10">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
            ...
            <div className="lg:col-span-6 space-y-6">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-white">
                Mevcut Akademik ve Günlük Araçlarınızla Uyumlu
              </h2>
              <p className="text-slate-400 text-sm md:text-base leading-relaxed">
                Rezervo, okul e-postanız ve takvim sisteminizle otomatik olarak entegre olur. Görüşme onaylandığında takviminize otomatik işlenir ve Zoom/Teams toplantı linkleri anında oluşturulur.
              </p>
              
              <div className="grid grid-cols-2 gap-4 pt-4">
                {[
                  { icon: Mail, title: "Anında E-posta", desc: "Randevu talepleri anında e-postanıza düşer." },
                  { icon: Video, title: "Otomatik Link", desc: "Zoom/Teams davetiyeleri kendiliğinden oluşur." },
                  { icon: Globe, title: "Çevrimdışı/Çevrimiçi", desc: "Ofis içi veya uzaktan görüşme desteği." },
                  { icon: Award, title: "Akademik Takip", desc: "Dönemlik danışmanlık raporlama." }
                ].map((integ, i) => (
                  <div key={i} className="flex gap-3 items-start">
                    <span className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-blue-400 shrink-0">
                      <integ.icon className="w-4 h-4" />
                    </span>
                    <div>
                      <h4 className="text-xs font-semibold text-white">{integ.title}</h4>
            ...
            {/* Right: Gorgeous Rotating Interactive Orbit Grid */}
            <div className="lg:col-span-6 flex items-center justify-center relative min-h-[350px]">
              
              {/* Outer Orbit Ring */}
              <div className="w-72 h-72 md:w-80 md:h-80 rounded-full border border-white/5 absolute animate-spin duration-[25s] linear" />
              ...
              {/* Orbiting Icons */}
              {[
                { icon: Mail, color: 'text-rose-400 bg-rose-500/10 border-rose-500/20', pos: 'top-10 left-10 md:top-8 md:left-12' },
                { icon: Video, color: 'text-blue-400 bg-blue-500/10 border-blue-500/20', pos: 'bottom-12 left-8 md:bottom-10 md:left-10' },
                { icon: Calendar, color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20', pos: 'top-8 right-12 md:top-6 md:right-16' },
                { icon: MessageSquare, color: 'text-purple-400 bg-purple-500/10 border-purple-500/20', pos: 'bottom-16 right-10 md:bottom-12 md:right-12' },
                { icon: Globe, color: 'text-amber-400 bg-amber-500/10 border-amber-500/20', pos: 'top-1/2 -left-6 md:-left-8' }
              ].map((orbit, i) => (
                <motion.div
                  key={i}
                  animate={{ y: [0, -6, 0] }}
                  transition={{ duration: 4, repeat: Infinity, delay: i * 0.5, ease: "easeInOut" }}
                  className={`absolute w-12 h-12 rounded-xl border flex items-center justify-center shadow-lg backdrop-blur-md z-20 ${orbit.color} ${orbit.pos}`}
                >
                  <orbit.icon className="w-5 h-5" />
                </motion.div>
              ))}
